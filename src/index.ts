...
import { OnlineFixEngine } from './onlinefix';
import { FaresEngine } from './fares';
import { searchSteamGames, popularGames } from './steam';
import { GameScraperEngine } from './gamescraper';
...
if (!BOT_TOKEN || !CLIENT_ID || !GUILD_ID) { console.error('Missing env: BOT_TOKEN, CLIENT_ID, GUILD_ID'); process.exit(1); }
if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });

// ==========================================
// TYPES
// ==========================================
interface RollConfig  { cost: number; }
interface RepoAlerts  { channelId: string | null; lastTag: string | null; }
interface Giveaway    { messageId: string; channelId: string; hostId: string; prize: string; title: string; endAt: string; winnerCount: number; entries: string[]; ended: boolean; }
interface BoostEntry  { type: 'luck' | 'daily'; expiresAt: string; }
interface DailyRolls  { date: string; count: number; }
interface AdToken     { userId: string; expiresAt: number; claimed: boolean; createdAt: number; }

// ==========================================
// PERSISTENCE
// ==========================================
let balancesCache: Record<string, number> = {};
let dailyCache:    Record<string, string> = {};
let rollConfig:    RollConfig  = { cost: 50 };
let repoAlerts:    RepoAlerts  = { channelId: null, lastTag: null };
let giveaways:     Record<string, Giveaway> = {};
let boosts:        Record<string, BoostEntry[]> = {};
let dailyRolls:    Record<string, DailyRolls> = {};
let bonusRolls:    Record<string, number> = {};

const adTokens = new Map<string, AdToken>(); // in-memory only, no persistence needed

function loadAll() {
    const load = <T>(file: string, def: T): T => {
...
const save = (file: string, data: any) => fs.writeFileSync(file, JSON.stringify(data, null, 2));
...
function loadCookies(): string {
    try { if (fs.existsSync(COOKIES_FILE)) return JSON.parse(fs.readFileSync(COOKIES_FILE,'utf8')).cookies || ''; } catch {}
    return '';
}
function saveCookies(cookies: string, by: string): void {
    fs.writeFileSync(COOKIES_FILE, JSON.stringify({ cookies, updatedAt: new Date().toISOString(), updatedBy: by }, null, 2));
}
OnlineFixEngine.configure(INDEX_FILE, loadCookies);
...
function getBalance(userId: string): number {
    if (isOwner(userId)) return 999_999_999_999;
    if (!(userId in balancesCache)) { balancesCache[userId] = 10; saveBalances(); }
    return balancesCache[userId];
}
function updateBalance(userId: string, amount: number): number {
    if (isOwner(userId)) return 999_999_999_999;
...
function getRollsUsedToday(userId: string): number {
    if (isOwner(userId)) return 0;
    const rec = dailyRolls[userId];
    if (!rec || rec.date !== todayStr()) return 0;
    return rec.count;
}
function incrementDailyRolls(userId: string): number {
    if (isOwner(userId)) return 0;
...
function getBonusRolls(userId: string): number {
    if (isOwner(userId)) return 999;
    return bonusRolls[userId] || 0;
}
function addBonusRolls(userId: string, amount: number) {
    bonusRolls[userId] = (bonusRolls[userId] || 0) + amount;
    saveBonusRolls();
}
function consumeBonusRoll(userId: string): boolean {
    if (isOwner(userId)) return true;
    if ((bonusRolls[userId] || 0) <= 0) return false;
    bonusRolls[userId]--;
    saveBonusRolls();
    return true;
}
...
function createAdToken(userId: string): string {
    // Invalidate any previous unclaimed token for this user
    for (const [tok, entry] of adTokens) {
        if (entry.userId === userId && !entry.claimed) adTokens.delete(tok);
    }
...
function adWatchUrl(userId: string): string {
...
setInterval(() => {
    for (const [tok, entry] of adTokens) {
        if (Date.now() > entry.expiresAt) adTokens.delete(tok);
    }
}, 5 * 60_000);
...
// BOOSTS
// ==========================================
function getActiveBoosts(userId: string): BoostEntry[] {
    const now = Date.now();
    boosts[userId] = (boosts[userId] || []).filter(b => new Date(b.expiresAt).getTime() > now);
    return boosts[userId];
}
function addBoost(userId: string, type: 'luck' | 'daily', hours = 24) {
    const expiresAt = new Date(Date.now() + hours * 3_600_000).toISOString();
    boosts[userId] = [...(boosts[userId] || []), { type, expiresAt }];
    saveBoosts();
}
function consumeBoost(userId: string, type: 'luck' | 'daily'): boolean {
...
const embed   = (title: string, color: number) => new EmbedBuilder().setTitle(title).setColor(color).setTimestamp();
const isAdmin = (i: ChatInputCommandInteraction) => i.memberPermissions?.has(PermissionFlagsBits.Administrator) ?? false;
const isWeekend = () => { const d = new Date().getDay(); return d === 0 || d === 6; };

async function fetchSteam(appId: string): Promise<any> {
    const r = await axios.get(`https://store.steampowered.com/api/appdetails?appids=${appId}&l=english`, { timeout: 5000 });
    if (!r.data[appId]?.success) throw new Error(`Steam App ID \`${appId}\` not found`);
    return r.data[appId].data;
}

const autoCache = new Map<string, { data: any; time: number }>();
async function cachedAuto(key: string, fetcher: () => Promise<any>, ttl = 30000): Promise<any> {
...
async function genAutocomplete(focused: string) {
    const choices: { name: string; value: string }[] = [];
    const seen = new Set<string>();
    const add = (id: string, name: string) => { if (!seen.has(id) && choices.length < 25) { seen.add(id); choices.push({ name: `${name} (${id})`.slice(0, 100), value: id }); } };
    try {
        const data = await cachedAuto(`gen_${focused}`, async () => { await FaresEngine.loadCatalog(); return { fares: FaresEngine.searchCatalog(focused, 15), steam: await searchSteamGames(focused, 15) }; });
        data.fares.forEach((g: any) => add(g.id, g.name));
        data.steam.forEach((g: any) => add(g.id, g.name));
    } catch {}
    if (!choices.length) for (const g of popularGames()) add(g.id, g.name);
    return choices;
}
async function gameAutocomplete(focused: string) {
    const choices: { name: string; value: string }[] = [];
    const seen = new Set<string>();
    const add = (n: string) => { if (!seen.has(n.toLowerCase()) && choices.length < 25) { seen.add(n.toLowerCase()); choices.push({ name: n.slice(0, 100), value: n }); } };
    if (focused) { try { (await cachedAuto(`game_${focused}`, async () => await searchSteamGames(focused, 25))).forEach((g: any) => add(g.name)); } catch {} }
    else for (const g of popularGames()) add(g.name);
    return choices;
}
...
// HTML PAGES
// ==========================================
function adPageHtml(token: string, valid: boolean, alreadyClaimed = false): string {
...
app.get('/', (_req: Request, res: Response) => res.send(landingPageHtml()));
app.get('/privacy', (_req: Request, res: Response) => res.send(privacyPageHtml()));
...
// Ad watch endpoint (for the desktop app — generates a live token so the full ad page works)
app.get('/ad/watch', (_req: Request, res: Response) => {
    const token = crypto.randomBytes(16).toString('hex');
    adTokens.set(token, { userId: 'app', expiresAt: Date.now() + AD_TOKEN_TTL, claimed: false });
    setTimeout(() => adTokens.delete(token), AD_TOKEN_TTL + 5000);
    // Serve the same adPageHtml but with a custom finish that signals the desktop app
    const valid = true;
    const claimed = false;
    const hasAdSense = ADSENSE_PUB_ID && !ADSENSE_PUB_ID.includes('REPLACE');
    const clientId = ADSENSE_PUB_ID || '';
...
<style>
*{box-sizing:border-box;margin:0;padding:0}
body{background:#0d1117;color:#e6edf3;font-family:'Segoe UI',system-ui,sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:20px;margin:0}
.card{background:#161b22;border:1px solid #30363d;border-radius:12px;padding:32px;max-width:460px;width:100%;text-align:center}
.logo{font-size:3rem;margin-bottom:8px}
h1{font-size:1.5rem;font-weight:700;margin-bottom:4px;color:#f0f6fc}
.subtitle{color:#8b949e;font-size:.9rem;margin-bottom:20px}
.ad-zone{background:#21262d;border:2px dashed #30363d;border-radius:8px;padding:20px;min-height:160px;display:flex;align-items:center;justify-content:center;margin-bottom:20px;color:#8b949e;font-size:.9rem}
#timer-bar{background:#21262d;border-radius:8px;height:8px;overflow:hidden;margin-bottom:12px}
#timer-fill{background-gradient:linear-gradient(90deg,#58a6ff,#79c0ff);background:#58a6ff;height:100%;width:100%;transition:width 1s linear}
#timer-text{color:#8b949e;font-size:.9rem;margin-bottom:20px}
#claim-btn{display:none;background:linear-gradient(135deg,#238636,#2ea043);color:#fff;border:none;border-radius:8px;padding:14px 32px;font-size:1.05rem;font-weight:700;cursor:pointer;width:100%;transition:transform .15s,box-shadow .15s}
#claim-btn:hover{transform:translateY(-1px);box-shadow:0 4px 16px rgba(35,134,54,.4)}
#claim-btn:disabled{opacity:.6;cursor:not-allowed;transform:none;box-shadow:none}
#reward-text{color:#8b949e;font-size:.8rem;margin-top:8px}
</style>
</head>
<body>
<div class="card">
  <div class="logo">🎲</div>
  <h1>Spoky's Bot</h1>
  <p class="subtitle">Watch an ad to earn a free bonus roll!</p>
  <div class="ad-zone">
    ${hasAdSense ? `
  ...
  <div id="timer-bar"><div id="timer-fill"></div></div>
  <div id="timer-text">Please wait <span id="secs">15</span>s…</div>
  <button id="claim-btn" onclick="claimAd('${token}')" disabled>⏳ Please wait…</button>
...
const text = document.getElementById('secs');
const btn  = document.getElementById('claim-btn');
const rtxt = document.getElementById('reward-text');
const interval = setInterval(() => {
  secs--;
  if (fill) fill.style.width = (secs / 15 * 100) + '%';
  if (text) text.textContent = secs;
...
async function claimAd(t) {
  btn.disabled = true;
  btn.textContent = '⏳ Claiming…';
  try {
    const r = await fetch('/ad/claim', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({token:t}) });
    const d = await r.json();
    if (d.ok) {
      rtxt.style.color = '#2ea043';
      rtxt.textContent = '✅ Bonus roll claimed! Head back to Discord.';
      btn.style.display = 'none';
      document.title = '_ad_done_';
    } else {
      rtxt.style.color = '#f85149';
      rtxt.textContent = '❌ ' + (d.error || 'Something went wrong.');
      btn.textContent = '🎁 Claim Your Free Roll';
      btn.disabled = false;
    }
  } catch(e) {
    rtxt.style.color = '#f85149';
    rtxt.textContent = '❌ Network error. Please try again.';
    btn.textContent = '🎁 Claim Your Free Roll';
    btn.disabled = false;
  }
}
...
app.post('/ad/claim', (req: Request, res: Response) => {
    ...
    const entry = adTokens.get(token);
    if (!entry)           return res.json({ ok: false, error: 'Invalid or expired token.' });
    if (Date.now() > entry.expiresAt) return res.json({ ok: false, error: 'Token expired. Use /watchad in Discord for a new link.' });
    if (entry.claimed)    return res.json({ ok: false, error: 'Already claimed.' });
    entry.claimed = true;
...
const commands = [
    ...
    // Games
    new SlashCommandBuilder().setName('gen').setDescription('Get Steam manifests as a zip')
        .addStringOption(o => o.setName('appid').setDescription('Steam App ID or game name').setRequired(true).setAutocomplete(true)),
    new SlashCommandBuilder().setName('game').setDescription('Search SteamRIP & DODI Repacks')
        .addStringOption(o => o.setName('name').setDescription('Game name').setRequired(true).setAutocomplete(true)),

    // Economy
    new SlashCommandBuilder().setName('balance').setDescription('Check your Gen balance'),
    new SlashCommandBuilder().setName('daily').setDescription('Claim your daily Gen'),
    new SlashCommandBuilder().setName('leaderboard').setDescription('Top Gen balances'),
    new SlashCommandBuilder().setName('give').setDescription('Give/remove Gen [Admin]')
        .addUserOption(o => o.setName('user').setDescription('User').setRequired(true))
        .addIntegerOption(o => o.setName('amount').setDescription('Amount (negative to remove)').setRequired(true)),

    // Gambling
    new SlashCommandBuilder().setName('roll').setDescription(`Roll the slots (${DAILY_ROLL_CAP}/day limit, costs Gen)`),
    new SlashCommandBuilder().setName('rollstatus').setDescription('Check your daily rolls remaining'),
    new SlashCommandBuilder().setName('watchad').setDescription(`Watch an ad to earn ${AD_REWARD} bonus roll(s)`),
    new SlashCommandBuilder().setName('roulette').setDescription('100 Gen for 10 random paid game manifests'),
    new SlashCommandBuilder().setName('setrolls').setDescription('Configure roll cost [Admin]')
        .addIntegerOption(o => o.setName('cost').setDescription('New roll cost in Gen').setRequired(true).setMinValue(1)),

    // Shop
    new SlashCommandBuilder().setName('shop').setDescription('Browse or buy from the Gen shop')
    ...
    // Admin
    new SlashCommandBuilder().setName('refreshindex').setDescription('Rebuild online-fix.me index [Admin]'),
    new SlashCommandBuilder().setName('updatecookies').setDescription('Update online-fix.me cookies [Admin]')
...
async function handleHelp(i: ChatInputCommandInteraction) {
    await i.reply({ embeds: [embed('📋 Command List', COLORS.purple)
        .addFields(
            { name: '🎮 Games',      value: '`/gen` `/game`' },
            { name: '💰 Economy',   value: '`/balance` `/daily` `/leaderboard` `/give`' },
            { name: '🎲 Gambling',  value: `\`/roll\` *(${DAILY_ROLL_CAP}/day)* \`/roulette\` \`/rollstatus\` \`/setrolls\`` },
            { name: '📺 Ad Rolls',  value: `\`/watchad\` — watch an ad for +${AD_REWARD} bonus roll(s)` },
            { name: '🛒 Shop',      value: '`/shop browse` `/shop buy`' },
            { name: '🎉 Giveaway',  value: '`/giveaway start` `/giveaway end` `/giveaway reroll`' },
            { name: '🔗 Repo',      value: '`/download` `/setrepoalerts`' },
            { name: '🔧 Admin',     value: '`/refreshindex` `/updatecookies`' },
        )], flags: [MessageFlags.Ephemeral] });
}

async function handleStats(i: ChatInputCommandInteraction) {
    await i.deferReply();
    const e = embed('📊 Bot Stats', COLORS.cyan).addFields(
        { name: 'Roll cost',        value: `${rollConfig.cost} Gen`,                                          inline: true },
        { name: 'Daily roll cap',   value: `${DAILY_ROLL_CAP} rolls/day`,                                     inline: true },
        { name: 'Ad bonus',         value: `${AD_REWARD} roll(s) per ad`,                                     inline: true },
        { name: 'Economy players',  value: `${Object.keys(balancesCache).length}`,                            inline: true },
        { name: 'Active giveaways', value: `${Object.values(giveaways).filter(g => !g.ended).length}`,        inline: true },
        { name: 'Repo alerts',      value: repoAlerts.channelId ? `<#${repoAlerts.channelId}>` : 'Not set',   inline: true },
    );
    await i.editReply({ embeds: [e] });
}

async function handleGen(i: ChatInputCommandInteraction) {
    await i.deferReply({ flags: [MessageFlags.Ephemeral] });
    const appId = i.options.getString('appid', true).trim();
    let steamName = appId, steamImage: string | undefined;
    try { const g = await fetchSteam(appId); steamName = g.name; steamImage = g.header_image; } catch {}
    await i.editReply({ content: `Fetching manifests for **${steamName}**...` });

    const result = await FaresEngine.getManifests(appId);
    const e = embed(steamName, COLORS.purple).addFields({ name: 'App ID', value: `\`${appId}\``, inline: true });
    if (steamImage) e.setThumbnail(steamImage);
    if (result.error || !result.manifests.length) { e.addFields({ name: 'Manifests', value: result.error || 'No manifests found' }); return i.editReply({ embeds: [e] }); }
    ...
    try {
        const fixPage = await OnlineFixEngine.findGamePage(steamName);
        if (fixPage) {
            const fixInfo = await OnlineFixEngine.getDownloadInfo(fixPage);
            fixText = `**[🛠️ Fix page](${fixInfo.pageUrl})**\nPassword: \`${fixInfo.password}\``;
            if (fixInfo.downloads.length) fixText += `\n\n**Direct Links:**\n\`\`\`\n${fixInfo.downloads.map(d=>d.url).join('\n')}\n\`\`\`\n*⚠️ Open the webpage link first or you'll get a 401.*`;
        }
    } catch { fixText = 'Error fetching online-fix.'; }
    e.addFields({ name: 'Online-Fix (Multiplayer)', value: fixText, inline: false });

    const zip = await FaresEngine.createManifestZip(appId, result.manifests, { maxSizeMb: 24 });
    if (!zip) { e.addFields({ name: 'Zip', value: 'Creation failed' }); return i.editReply({ embeds: [e] }); }
...
async function handleGame(i: ChatInputCommandInteraction) {
    await i.deferReply();
    const query = i.options.getString('name', true).trim().replace(/[™®©]/g, '').trim();
    await i.editReply({ content: `Searching **${query}**...` });
    const scraped = await GameScraperEngine.scrapeByQuery(query);
    const e = embed(query, COLORS.blue);
    if (scraped.steamrip) e.addFields({ name: 'SteamRIP',    value: `[Webpage](${scraped.steamrip.pageUrl})\n${scraped.steamrip.downloads.map(d=>`[${d.label}](${d.url})`).join('\n')||'No links'}`.slice(0,1024) });
    if (scraped.dodi)     e.addFields({ name: 'DODI Repacks', value: `[Webpage](${scraped.dodi.pageUrl})\n${scraped.dodi.downloads.map(d=>`[${d.label}](${d.url})`).join('\n')||'No links'}`.slice(0,1024) });
    if (!scraped.steamrip && !scraped.dodi) return i.editReply({ embeds: [embed('No Results', COLORS.yellow).setDescription('Try a different spelling.')] });
    await i.editReply({ embeds: [e] });
}
...
async function handleBalance(i: ChatInputCommandInteraction) {
    ...
    await i.reply({ embeds: [embed(`${i.user.username}'s Balance`, COLORS.gold)
        ...
        .addFields(
            { name: 'Rolls today', value: `${used}/${DAILY_ROLL_CAP}`, inline: true },
            { name: 'Bonus rolls', value: `${isOwner(i.user.id) ? '∞' : bonus}`, inline: true },
        )] });
}
...
async function handleLeaderboard(i: ChatInputCommandInteraction) {
    ...
    if (!sorted.length) return i.editReply({ content: 'No one has any Gen yet.' });
    const e = embed('🏆 Gen Leaderboard', COLORS.gold);
    let desc = '';
    for (let idx = 0; idx < sorted.length; idx++) desc += `**${idx+1}.** <@${sorted[idx][0]}> — **${sorted[idx][1]} Gen**\n`;
...
async function handleDaily(i: ChatInputCommandInteraction) {
    ...
    if (last && (Date.now()-last.getTime())/3_600_000 < 24)
        return i.reply({ content: `Already claimed. Try again in **${(24-(Date.now()-last.getTime())/3_600_000).toFixed(1)} hours**.`, flags: [MessageFlags.Ephemeral] });
    dailyCache[i.user.id] = new Date().toISOString(); saveDaily();
    let reward = 10;
...
async function handleGive(i: ChatInputCommandInteraction) {
    if (!isAdmin(i) && !isOwner(i.user.id)) return i.reply({ content: 'Administrator required.', flags: [MessageFlags.Ephemeral] });
...
async function handleRollStatus(i: ChatInputCommandInteraction) {
    const used = getRollsUsedToday(i.user.id);
    ...
    const e = embed('🎲 Roll Status', COLORS.blue)
        .addFields(
            { name: 'Daily rolls used',      value: `${used}/${DAILY_ROLL_CAP}`, inline: true },
            { name: 'Daily rolls remaining', value: `${isOwner(i.user.id)?'∞':remaining}`, inline: true },
            { name: 'Bonus rolls banked',    value: `${isOwner(i.user.id)?'∞':bonus}`,     inline: true },
        )
        .setFooter({ text: 'Earn extra rolls by watching an ad with /watchad or the button below' });

    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setLabel(`📺 Watch Ad (+${AD_REWARD} Roll)`).setStyle(ButtonStyle.Link).setURL(url)
    );
    await i.reply({ embeds: [e], components: [row], flags: [MessageFlags.Ephemeral] });
}

async function handleRoll(i: ChatInputCommandInteraction) {
    await i.deferReply();
    ...
    if (!isOwner(i.user.id) && used >= DAILY_ROLL_CAP && bonus <= 0) {
        ...
        const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
            new ButtonBuilder().setLabel(`📺 Watch Ad for +${AD_REWARD} Roll`).setStyle(ButtonStyle.Link).setURL(url)
        );
        return i.editReply({ embeds: [e], components: [row] });
    }

    const cost = rollConfig.cost;
    if (!isOwner(i.user.id)) updateBalance(i.user.id, -cost);

    // Use a bonus roll if over the daily cap
    const usedBonus = !isOwner(i.user.id) && used >= DAILY_ROLL_CAP;
    if (usedBonus) consumeBonusRoll(i.user.id);
    ...
    for (const o of outcomes) { rand -= o.weight; if (rand <= 0) { chosen = o; break; } }

    if (hasLuck) consumeBoost(i.user.id, 'luck');
    ...
    const e = embed('🎰 Slot Roll', color)
        .addFields(
            { name: 'Result',       value: `${chosen.emoji} **${chosen.label}** — ${chosen.value>0?'+':''}${chosen.value} Gen`, inline: true },
            { name: 'Cost paid',    value: `${cost} Gen${usedBonus?' *(bonus roll)*':''}`, inline: true },
            { name: 'New Balance',  value: `**${isOwner(i.user.id)?'∞':newBal} Gen**`, inline: true },
            { name: 'Rolls left today', value: `${isOwner(i.user.id)?'∞':rollsLeft}/${DAILY_ROLL_CAP}`, inline: true },
            { name: 'Bonus rolls',  value: `${isOwner(i.user.id)?'∞':bonusLeft}`, inline: true },
        )
...
async function handleWatchAd(i: ChatInputCommandInteraction) {
    const url = adWatchUrl(i.user.id);
    const used = getRollsUsedToday(i.user.id);
    const bonus = getBonusRolls(i.user.id);
    const e = embed('📺 Watch Ad for Bonus Rolls', COLORS.orange)
        ...
        .addFields(
            { name: 'Daily rolls used',  value: `${used}/${DAILY_ROLL_CAP}`, inline: true },
            { name: 'Bonus rolls banked',value: `${bonus}`,                  inline: true },
        );
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setLabel(`📺 Watch Ad (+${AD_REWARD} Roll)`).setStyle(ButtonStyle.Link).setURL(url)
    );
...
async function handleRoulette(i: ChatInputCommandInteraction) {
    await i.deferReply({ flags: [MessageFlags.Ephemeral] });
    const cost = 100;
    if (!isOwner(i.user.id)) updateBalance(i.user.id, -cost);
    await i.editReply({ content: '🎰 Spinning… fetching 10 random paid game manifests!' });
    const zip = new AdmZip(); let count = 0; let attempts = 0;
    while (count < 10 && attempts < 30) {
        ...
        try {
            ...
            if (result.manifests?.length) {
                ...
                const buf = await FaresEngine.downloadManifest(appId, m.depotId, m.manifestId);
                if (buf.length < 2*1024*1024) { zip.addFile(`${appId}_${m.depotId}.manifest`, buf); count++; }
            }
        } catch {}
    }
    if (count === 0) { if (!isOwner(i.user.id)) updateBalance(i.user.id, cost); return i.editReply({ content: 'Failed to fetch games. Refunded 100 Gen.' }); }
...
async function handleSetRolls(i: ChatInputCommandInteraction) {
    if (!isAdmin(i) && !isOwner(i.user.id)) return i.reply({ content: 'Administrator required.', flags: [MessageFlags.Ephemeral] });
...
async function handleShopBrowse(i: ChatInputCommandInteraction) {
...
async function handleShopBuy(i: ChatInputCommandInteraction) {
    const itemId = i.options.getString('item', true);
    const item = SHOP_ITEMS.find(s => s.id === itemId);
    if (!item) return i.reply({ content: 'Unknown item.', flags: [MessageFlags.Ephemeral] });
    const bal = getBalance(i.user.id);
    if (!isOwner(i.user.id) && bal < item.price)
        return i.reply({ content: `You need **${item.price} Gen** but have **${bal} Gen**.`, flags: [MessageFlags.Ephemeral] });
    if (!isOwner(i.user.id)) updateBalance(i.user.id, -item.price);
    if (item.id === 'luck_boost')  addBoost(i.user.id, 'luck', 24);
    if (item.id === 'daily_boost') addBoost(i.user.id, 'daily', 48);
    if (item.id === 'extra_rolls') addBonusRolls(i.user.id, 5);
    if (item.id === 'jackpot_key') addBoost(i.user.id, 'luck', 2);
...
// GIVEAWAY
// ==========================================
async function handleGiveawayStart(i: ChatInputCommandInteraction) {
    await i.deferReply();
    ...
    const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
        new ButtonBuilder().setCustomId('giveaway:enter').setLabel('🎉 Enter').setStyle(ButtonStyle.Primary)
    );
...
async function endGiveaway(msgId: string, client: Client, _forced=false) {
    const g = giveaways[msgId];
    if (!g || g.ended) return;
    g.ended = true; saveGiveaways();
    try {
        const ch = await client.channels.fetch(g.channelId) as TextChannel;
        const msg = await ch.messages.fetch(g.messageId);
        let desc = `**Prize:** ${g.prize}\n`;
        if (!g.entries.length) { desc += '\n❌ No entries.'; }
        else {
...
async function handleGiveawayEnd(i: ChatInputCommandInteraction) {
    if (!isAdmin(i) && !isOwner(i.user.id)) return i.reply({ content:'Administrator required.', flags:[MessageFlags.Ephemeral] });
    const msgId = i.options.getString('message_id', true).trim();
    const g = giveaways[msgId];
    if (!g) return i.reply({ content:'Not found.', flags:[MessageFlags.Ephemeral] });
    if (g.ended) return i.reply({ content:'Already ended.', flags:[MessageFlags.Ephemeral] });
    await endGiveaway(msgId, i.client, true);
    await i.reply({ content:'✅ Giveaway ended.', flags:[MessageFlags.Ephemeral] });
}

async function handleGiveawayReroll(i: ChatInputCommandInteraction) {
    if (!isAdmin(i) && !isOwner(i.user.id)) return i.reply({ content:'Administrator required.', flags:[MessageFlags.Ephemeral] });
    const msgId = i.options.getString('message_id', true).trim();
    const g = giveaways[msgId];
    if (!g || !g.ended || !g.entries.length) return i.reply({ content:'Cannot reroll this giveaway.', flags:[MessageFlags.Ephemeral] });
...
async function fetchLatestRelease() {
    const r = await axios.get(`https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/releases/latest`, { timeout:8000, headers:{'User-Agent':'DiscordBot/1.0', Accept:'application/vnd.github+json'} });
    return r.data;
}
async function handleDownload(i: ChatInputCommandInteraction) {
    await i.deferReply();
    try {
        ...
        const e = embed(`📦 ${APP_DISPLAY_NAME} ${rel.tag_name}`, COLORS.cyan)
            ...
            .setURL(rel.html_url);
        if (rel.assets?.length) e.addFields({ name:'⬇️ Downloads', value:rel.assets.map((a:any)=>`[${a.name}](${a.browser_download_url}) *(${(a.size/1_048_576).toFixed(1)} MB)*`).join('\n').slice(0,1024) });
        else e.addFields({ name:'⬇️ Downloads', value:`[View Release](${rel.html_url})` });
        await i.editReply({ embeds:[e] });
    } catch (err:any) { await i.editReply({ content:`Failed: ${err.message}` }); }
}

async function handleSetRepoAlerts(i: ChatInputCommandInteraction) {
    if (!isAdmin(i) && !isOwner(i.user.id)) return i.reply({ content:'Administrator required.', flags:[MessageFlags.Ephemeral] });
    const ch = i.options.getChannel('channel', true);
    repoAlerts.channelId = ch.id; saveRepoAlerts();
    await i.reply({ embeds:[embed('✅ Repo Alerts Set', COLORS.green).setDescription(`Updates for **${REPO_OWNER}/${REPO_NAME}** → <#${ch.id}>`)] });
}

async function pollRepoUpdates(client: Client) {
    if (!repoAlerts.channelId) return;
    try {
        const rel = await fetchLatestRelease();
    ...
    } catch (err:any) { console.error('[RepoPoller]', err.message); }
}

async function handleRefreshIndex(i: ChatInputCommandInteraction) {
    await i.deferReply({ flags:[MessageFlags.Ephemeral] });
    if (!isAdmin(i) && !isOwner(i.user.id)) return i.editReply({ content:'Admin required.' });
    await i.editReply({ content:'Building index...' });
    const idx = await OnlineFixEngine.buildIndex(()=>{});
    await i.editReply({ embeds:[embed('Index Refreshed', COLORS.green).addFields({ name:'Games indexed', value:`${idx.gameCount}` })] });
}
async function handleUpdateCookies(i: ChatInputCommandInteraction) {
    await i.deferReply({ flags:[MessageFlags.Ephemeral] });
    if (!isAdmin(i) && !isOwner(i.user.id)) return i.editReply({ content:'Admin required.' });
...
client.once(Events.ClientReady, async () => {
    ...
    } catch (e) { console.error('Cmd registration failed', e); }
...
client.on('interactionCreate', async (i) => {
    ...
    if (i.isAutocomplete()) {
        try {
            if (i.commandName==='gen')  return i.respond(await genAutocomplete(i.options.getFocused()));
            if (i.commandName==='game') return i.respond(await gameAutocomplete(i.options.getFocused()));
        } catch {}
        return i.respond([]);
    }
    ...
    if (i.isButton() && i.customId==='giveaway:enter') {
        ...
        if (!g || g.ended) return btn.reply({ content:'This giveaway has ended.', flags:[MessageFlags.Ephemeral] });
        if (g.entries.includes(btn.user.id)) return btn.reply({ content:'You are already entered!', flags:[MessageFlags.Ephemeral] });
        g.entries.push(btn.user.id); saveGiveaways();
        return btn.reply({ content:`✅ Entered! **${g.entries.length}** total entries.`, flags:[MessageFlags.Ephemeral] });
    }
    ...
    try {
        switch (i.commandName) {
            ...
            case 'stats':         await handleStats(i);          break;
            case 'gen':           await handleGen(i);            break;
            case 'game':          await handleGame(i);           break;
            case 'balance':       await handleBalance(i);        break;
            case 'daily':         await handleDaily(i);          break;
            case 'leaderboard':   await handleLeaderboard(i);    break;
            case 'give':          await handleGive(i);           break;
            case 'roll':          await handleRoll(i);           break;
            case 'rollstatus':    await handleRollStatus(i);     break;
            case 'watchad':       await handleWatchAd(i);        break;
            case 'roulette':      await handleRoulette(i);       break;
            case 'setrolls':      await handleSetRolls(i);       break;
            case 'download':      await handleDownload(i);       break;
            case 'setrepoalerts': await handleSetRepoAlerts(i);  break;
            case 'refreshindex':  await handleRefreshIndex(i);   break;
            case 'updatecookies': await handleUpdateCookies(i);  break;
            case 'shop':
                if (i.options.getSubcommand()==='browse') await handleShopBrowse(i);
                if (i.options.getSubcommand()==='buy')    await handleShopBuy(i);
                break;
            case 'giveaway':
                if (i.options.getSubcommand()==='start')  await handleGiveawayStart(i);
                if (i.options.getSubcommand()==='end')    await handleGiveawayEnd(i);
                if (i.options.getSubcommand()==='reroll') await handleGiveawayReroll(i);
                break;
        }
    } catch (e:any) {
